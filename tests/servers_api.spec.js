...
test(`Servers API v${API_VERSION}`, async ({ request }) => {
  ...
  // --- SERVERS ---
  const serversResponse = await request.post(`${baseURL}/api/${API_VERSION}/servers`, {
    headers: {
  ...
  console.log("SERVERS:", serversBody);

  expect(serversResponse.status()).toBe(200);

  // проверяем структуру ответа
  expect(serversBody).toHaveProperty('result');
  expect(serversBody.result).toHaveProperty('list');
